...
public class sequenceMutiTypeEntity extends AbstractHttpEntity {
	...
	private ProcessCallBack mProcessCb;
		...
		for(int i = 0; i < mParamList.size(); i++){
        	...
        	if(p.mType == TYPE_BYTEARRAY){
                ...
                if (p.mLength < 0) {
                    ...
                    while ((l = instream.read(buffer)) != -1) {
                    	if(mProcessCb != null){
                    		mProcessCb.onProcess(p.mLength, 0);
                    	}
                    ...
                    while (remaining > 0) {
                        ...
                        if(mProcessCb != null){
                    		mProcessCb.onProcess((int)(p.mLength - remaining), p.mLength);
                    	}
	...
	public void setProcessCb(ProcessCallBack cb){
		mProcessCb = cb;
	}
