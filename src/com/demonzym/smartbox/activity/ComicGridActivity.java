 ...
 *
 */
public class ComicGridActivity extends ActivityBase{	

	private ArrayList<Foobar> mComicData = new ArrayList<Foobar>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		waitingSomething(R.string.wait_for_getting_data);
	...
	private void iniViews() {
		...
		mAdapter = new ComicAdapter(width,height);
		mGrid.setAdapter(mAdapter);
		mGrid.setOnItemClickListener(new ItemClickListener());
		

		mImageView = (ImageView) findViewById(R.id.imageView1);
		mImageView.setImageResource(R.drawable.zongyi);
	...
	private void iniData() {
		...
		Api.getListInZy(mData, new IHttpListener() {
			...
			public void onSuccess(int requestId, String data, IHttp http) {
				...
				for(int i = 0; i < mld.mFoobarList.size(); i++){
					mComicData.add(mld.mFoobarList.get(i));
				}
	...
	class ComicAdapter extends BaseAdapter{
		private int width=0;
		private int height=0;
		
		public ComicAdapter(int width,int height){
			this.width=width;
			this.height=height;
...
//			});
		...
		public void initFetcher() {
			mImageFetcher = new ImageFetcher(ComicGridActivity.this, width, height);
...
//					if(md!=null){
						Intent intent = new Intent(ComicGridActivity.this,
						...
						isLoading = false;
//					}else{
//						Toast.makeText(ComicGridActivity.this, R.string.error_for_get_data, Toast.LENGTH_LONG).show();
//				
//					}					
				}
